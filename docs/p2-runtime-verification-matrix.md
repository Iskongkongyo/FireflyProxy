...
当前 P2 门禁是 P1 的严格超集：先执行全部 P0/P1 测试、lint、生产构建和 Browser Core E2E，再用 Playwright Core 驱动本机 Chromium 运行 Runtime Bridge 专项页面。upstream、跨 origin DNS 与真实连接均由本地动态端口 Fixture 提供，不访问公网或修改系统 hosts。
...
| 安全内核不旁路 | 所有 HTTP(S) 映射结果进入现有 Canonical Route，继续执行 URL、DNS SSRF、Pinning、Header/Cookie 与资源限制 |
...
- WebSocket 构造器与 ws/wss Upgrade 尚未纳入 4.2，必须等待 4.3 的服务端安全代理一起启用。
- 服务端 Cookie Jar 仍不能模拟 `document.cookie`；多个 upstream 仍共享 Browser Proxy origin，Origin Isolation 留待 4.4。

## 发布判定

只有 P1 子门禁和 Runtime Bridge Playwright E2E 均输出 PASS，才满足当前 4.2 验收。该结果证明常见 SPA 动态 HTTP/SSE URL 可映射，不代表 WebSocket、高级 Service Worker/Worker、CSP 特例或完整浏览器隔离已经完成。
