...
> 状态：✅ 已于 2026-08-30 完成路线图 4.2，并在 4.3 接入 WebSocket 包装。HTML `<head>` 最前方注入唯一 `data-proxyweb-runtime` 脚本，映射 Request/fetch、XHR、EventSource、WebSocket、window.open 与 History；相对 URL 同时遵循 upstream 文档地址和有效 `<base>`。全局、Session 或 HTML Rewrite 任一关闭都会禁止注入和脚本交付，WebSocket 包装还要求服务端能力有效。
...
> 状态：✅ 已于 2026-08-30 完成路线图 4.3。`ws@8.21.3` 接管 Upgrade；下游 101 只在上游握手、DNS SSRF、请求级 Pinning/TLS 与远端地址校验成功后完成。Runtime 使用签名且握手后移除的来源标记还原 upstream Origin，应用子协议、Cookie Jar、文本/二进制、关闭码、ping/pong、backpressure、payload/idle/连接数上限和 shutdown 回收均已接入自动化与真实 Edge 门禁。
...
> 状态：✅ 已于 2026-08-30 完成路线图 3.7，并在 4.2–4.3 激活 Runtime Bridge 与 WebSocket Proxy 开关。HTML/CSS Rewrite、Runtime Bridge、WebSocket Proxy、Cookie Jar 与 Compatibility Headers 位于高级折叠面板；参数绑定 Browser Session 且只能收紧服务器全局配置，不能启用后端全局关闭的能力。
